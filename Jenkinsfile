...
stage("Debugging") {
    steps {
        script {
            ...
            sh "sudo ls -l /home/vagrant/oracle/wlserver/common/bin/wlst.sh"
        }
    }
}
...
stage("Deploy to WebLogic") {
    steps {
        script {
            ...
            sh """
                sudo \${WLS_HOME}/common/bin/wlst.sh -i -Dweblogic.management.username=weblogic -Dweblogic.management.password=Wevioo@2023++++ <<-EOF
